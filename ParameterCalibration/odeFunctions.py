# Import all packages
from sympy import symbols, Piecewise, exp, Matrix, core, diff, tanh
import numpy as np
import pandas as pd
...
def differencePreData(pLog, argP):
    ...
    if len(doseList) == 0 and len(plateID_list) == 0:
        dataObs = obsSimu_allDoses.reshape(Nm * Nt, )
        dataObs[np.isnan(dataObs)] = np.inf
        difference = ((dataObs - dataM))
    elif len(doseList) == 0 and len(plateID_list) > 0:
        dataObs = obsSimu_allDoses.reshape(Nm * Nt, )
        dataObs[np.isnan(dataObs)] = np.inf
        difference = ((dataObs - dataM) / dataS)
    elif len(doseList) > 0 and len(plateID_list) == 0:
        dataObs = obsSimu_allDoses.reshape(Nm * Nt * len(doseList), )
        dataObs[np.isnan(dataObs)] = np.inf
        difference = ((dataObs - dataM))
    elif len(doseList) > 0 and len(plateID_list) > 0:
        dataObs = obsSimu_allDoses.reshape(Nm * Nt * len(doseList), )
        ...
        difference = ((dataObs - dataM) / dataS)
        # print("obsSimu:")
        # print(obsSimu_allDoses.reshape(Nm * Nt * len(doseList), ))
        # print("difference:")
        # print(difference)
...
def multiesti(seed, argPplus):
    [pInit, argP, testRun, startTime, timeMax, path, DATE, MODEL, USER, bounds] = argPplus
    np.random.seed(seed)
    ...
    pathToCSVfile = path + DATE + USER + MODEL + "Model_FinishedRuns_costs.csv"
    ...
    try:
        outputDf.to_csv(path + DATE + USER + MODEL +"Model_parameterEstimates_parmset" + str(seed+1) + "_cost_" + "{0:.2f}".format(cost) + ".csv")
        print("Saved %s" % (path + DATE + USER + MODEL +"Model_parameterEstimates_parmset" + str(seed+1) + "_cost_" + "{0:.2f}".format(cost) + ".csv"))
    except:
        outputDf.to_csv(path + DATE + USER + MODEL +"Model_parameterEstimates_parmset" + str(seed+1) + "_cost_" + str(cost) + ".csv")
        print("Saved %s" % (path + DATE + USER + MODEL +"Model_parameterEstimates_parmset" + str(seed+1) + "_cost_" + str(cost) + ".csv"))
