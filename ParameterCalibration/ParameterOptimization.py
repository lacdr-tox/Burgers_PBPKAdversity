...
if __name__ == '__main__':
    ...
    if not useSysArgv:
        ...
        dataFile = "RATdata_NIT_oral_20250303.csv"
        odeFile = "NIT_rat_oral_20250304.txt"
   ...
   # Check if plateIDName exists in the data
    ...
    for cell_linei, celllinet in enumerate(obsStateVarList):
        ...
        dfTempM = np.nanmean(dfTemp, axis=0)
        dfTempS = np.nanstd(dfTemp, axis=0)
        dfTempMReal = np.nanmean(dfTempReal, axis=0)
        dfTempSReal = np.nanstd(dfTempReal, axis=0)
    ...
    if 0 in dataS:
        print("WARNING: dataS contains zero(s).")
        if sdzero == "Small_Sd":
            dataS[dataS==0] = sdsmall
            print("WARNING: zero(s) in dataS replaced by small value.")

        elif sdzero == "Large_Sd":
            dataS[dataS==0] = sdlarge
            print("WARNING: zero(s) in dataS replaced by large value.")

        elif sdzero == "Nan_Sd":
            dataS[dataS==0] = np.nan
            print("WARNING: zero(s) in dataS replaced by NaN.")

        elif sdzero == "Average_Sd":
            meansd = np.nanmean(dataS, axis = 1)
            meansd[meansd==0] = sdlarge #by default sd becomes large if all timepoints have 0 sd
            for cell_linei, celllinet in enumerate(obsStateVarList):
                dataS[cell_linei,:][dataS[cell_linei,:]==0] = meansd[cell_linei]

            print("WARNING: zero(s) in dataS replaced by average sd value.")

        elif sdzero == "Input_Sd":
            dataS[dataS==0] = sdvalue
            print("WARNING: zero(s) in dataS replaced by chosen value.")
    
    if np.nan in dataS:
        print("WARNING: dataS contains nan(s).")
    ...
    if stressExplicit:
        if stressInput == "fixed":
            ...
            for i, doseName in enumerate(doseNames):
                if doseFunctionType == "expDecay":
                    sText = sText + "(" + doseName + " * exp(-tsym * tau1), (dose >= " + str(doseList[i] - 0.00001) + ") & (dose <= " + str(doseList[i] + 0.00001) + ")),"
                elif doseFunctionType == "anaus":
                    sText = sText + "(" + doseNames[i] + " * 0.5 * (tanh(100*(tsym)) - tanh(100*(tsym-tau1)))" + ", (dose >= " + str(doseList[i] - 0.00001) + ") & (dose <= " + str(doseList[i] + 0.00001) + ")), "
            sText = sText + "(0, True))"
            exec(sText)
            print("Executed: %s" % sText)
            Settings = Settings + [sText]
   ...
   # Read in the ODEs from a file
    ...
    pathToCSVfile = path + DATETIME + USER + MODEL + "Model_FinishedRuns_costs.csv"
    ...
    argPplus = [pInitList, argP, testRun, t0, timeMax, path, DATETIME, MODEL, USER, bounds]
    ...
    for pIniti in range(0,len(pInitList)):
        ...
        try:
            ...
            plt.savefig(figPath + DATETIME + USER + MODEL + "Model_parmset" + str(pIniti + 1) + "_modelSimulationMeanData.pdf")
            plt.close()
            ...
            plt.tick_params(labelcolor='none', top=False, bottom=False, left=False, right=False)
            plt.xlabel("Time (h)", fontsize=28)
            plt.savefig(figPath + DATETIME + USER + MODEL + "Model_parmset" + str(pIniti+1) + "_modelSimulationReplicateData.pdf")
            plt.close()
            ...
            plt.tick_params(labelcolor='none', top=False, bottom=False, left=False, right=False)
            plt.xlabel("Time (h)", fontsize=28)
            plt.savefig(figPath + DATETIME + USER + MODEL +"Model_parmset" + str(pIniti+1) + "_InnerStates.pdf")
            plt.close()
    ...
    costdf.to_csv(path + DATETIME + USER + MODEL +"Model_costs.csv")
